...
\author Simon Gog
*/
#ifndef _BITMAGIC_HPP_
#define _BITMAGIC_HPP_
...
inline uint16_t bit_magic::max_excess(uint64_t max2, uint16_t &b1Cnt) {
		...
		b1Cnt = static_cast<uint16_t>((sum + (sum >> 32)) >> 1);
		// Maxima fuer 64bit Bloecke
		max2 = (max >> 32) + 0x0000000000000060ULL + (sum & 0x00000000FFFFFFFFULL);
		return static_cast<uint16_t>(((max2 - (max & 0x00000000FFFFFFFFULL)) & 0x0000000000000080ULL) ? max2 & 0x000000000000007FULL : max & 0x00000000FFFFFFFFULL);
...
inline uint16_t bit_magic::max_excess2(uint64_t x, uint16_t &b1Cnt) {
		...
		uint8_t maxi1 = static_cast<uint8_t>(_max_byte_excesses), maxi2 = static_cast<uint8_t>(_max_byte_excesses >> 32);
		for (uint8_t i = 1, m; i<4; ++i) {
			if ((m = static_cast<uint8_t>((_max_byte_excesses >>= 8))) > maxi1) maxi1 = m;
		}
		_max_byte_excesses >>= 8;
		for (uint8_t i = 5, m; i<8; ++i) {
			if ((m = static_cast<uint8_t>((_max_byte_excesses >>= 8))) > maxi2) maxi2 = m;
		}
...
inline uint8_t bit_magic::first_excess_position(uint64_t w, uint8_t excess_val, uint64_t &byte_prefix_sums_x_2) {
		...
		if (max_byte_excesses) {
			uint8_t block_of_occurence_x_8 = DeBruijn64ToIndex[(static_cast<uint64_t>((static_cast<long long>(max_byte_excesses) &- static_cast<long long>(max_byte_excesses)))*DeBruijn64) >> 58] & 0xF8;
...
inline uint8_t bit_magic::find_open(uint64_t w, uint8_t close_parenthesis_index) {
		...
		if (near_match) { // the matching opening parenthesis is at most 7 positions before the closing paranthesis
			...
			if (max_byte_excesses) {
				...
				max_byte_excesses -= (max_byte_excesses >> 8);
				assert(max_byte_excesses = static_cast<uint64_t>(static_cast<long long>(max_byte_excesses) &- static_cast<long long>(max_byte_excesses)));
...
inline uint16_t bit_magic::max_excess3(uint64_t x, uint16_t &b1Cnt) {
		...
		return maxi&0x7F;
		*/	uint8_t maxi[8] = { (uint8_t)(max_excess_8bit[x] | 0x80),
			(uint8_t)((uint8_t)(sum) + max_excess_8bit[x >> 8]),
			(uint8_t)((uint8_t)(sum >> 8) + max_excess_8bit[x >> 16]),
			(uint8_t)((uint8_t)(sum >> 16) + max_excess_8bit[x >> 24]),
			(uint8_t)((uint8_t)(sum >> 24) + max_excess_8bit[x >> 32]),
			(uint8_t)((uint8_t)(sum >> 32) + max_excess_8bit[x >> 40]),
			(uint8_t)((uint8_t)(sum >> 40) + max_excess_8bit[x >> 48]),
			(uint8_t)((uint8_t)(sum >> 48) + max_excess_8bit[x >> 56]) };
		...
		if (maxi[0] < maxi[2]) maxi[0] = maxi[2];
		if (maxi[4] < maxi[6]) maxi[4] = maxi[6];
		return static_cast<uint16_t>((maxi[0] < maxi[4]) ? maxi[4] & static_cast<uint8_t>(0x7F) : maxi[0] & static_cast<uint8_t>(0x7F));
}
...
inline uint32_t bit_magic::b10Cnt(uint64_t x, uint64_t &c) {
		uint32_t res = static_cast<uint32_t>(b1Cnt((x ^ ((x << 1) | c)) & (~x)));
		c = (x >> 63);
		return res;
}
...
inline uint32_t bit_magic::b10CntNaive(uint64_t x, uint64_t &c) {
		uint32_t sum = 0, lastbit = static_cast<uint32_t>(c);
...
inline uint32_t bit_magic::j1BP(uint64_t x, uint32_t j) {
		...
		return static_cast<uint32_t>(b) + (uint32_t)((((((((l - 1) * 0x0101010101010101ULL) | 0x8080808080808080ULL) - s) & 0x8080808080808080ULL) >> 7) * 0x0101010101010101ULL) >> 56);
}
...
inline const uint32_t bit_magic::r1BP(uint64_t x) {
		...
		//default:
		return DeBruijn64ToIndex[((static_cast<long long>(x) &- static_cast<long long>(x))*DeBruijn64) >> 58];
...
inline const uint64_t bit_magic::all11BPs(uint64_t x, bool &c) {
		uint64_t ex11 = (x&(x >> 1)) & 0x5555555555555555ULL;
		uint64_t ex10or01 = (ex11 | (ex11 << 1)) ^ x;
		ex11 += (((ex11 | (ex11 << 1)) + (((ex10or01 << 1) & 0x5555555555555555ULL) | (uint64_t)c)) & ((ex10or01 & 0x5555555555555555ULL) | ex11));
		c = (x&(~ex11)) >> 63;
		return ex11;
}
