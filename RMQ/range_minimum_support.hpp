...
*/
/*! \file range_minimum_support_sparse_table.hpp
\brief range_minimum_support_sparse_table.hpp contains the class range_minimum_support_sparse_table which supports range minimum or range maximum queries on a random access container in constant time and \f$\Order{n\log^2 n} bits\f$ space.
\author Simon Gog
*/
#ifndef _RANGE_MINIMUM_SUPPORT_HPP_
#define _RANGE_MINUMUM_SUPPORT_HPP_
...
#include <ostream>
	...
	class range_minimum_support_sparse_table;
	...
	struct range_maximum_support_sparse_table {
		typedef range_minimum_support_sparse_table<RandomAccessContainer, false> type;
	};

	//! A class to support range minimum or range maximum queries on a random access container.
	/*!
	* This class takes two template parameters. The first on is the type of random access container, for which
	* the range minimum/maximum support should be build. The second one specifies if the data structure
	...
	*      \f$ \Order{n\log^2 n} \f$ bits for data structure. We used bit compression to get a good result in practice.
	*/
	...
	class range_minimum_support_sparse_table {
		...
		void copy(const range_minimum_support_sparse_table& rm) {
			m_v = rm.m_v;
			m_k = rm.m_k;
			if (m_table != NULL) {
				delete[] m_table;
	...
	public:
		...
		range_minimum_support_sparse_table(const RandomAccessContainer *v = NULL) :m_v(v), m_k(0), m_table(NULL) {
			construct();
		}

		//! Copy constructor
		range_minimum_support_sparse_table(const range_minimum_support_sparse_table &rm) {
		...
		~range_minimum_support_sparse_table() {
			if (m_table != NULL)
				delete[] m_table;
		}

		range_minimum_support_sparse_table& operator=(const range_minimum_support_sparse_table &rm) {
			if (this != &rm) {
				copy(rm);
			}
			return *this;
