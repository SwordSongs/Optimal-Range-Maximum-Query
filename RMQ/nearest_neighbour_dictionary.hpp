...
#include <stdexcept>
	...
	//! Nearest neighbour dicitionry for sparse uniforme sets (described in Geary et al., A Simple Optimal Representation for Balanced Parentheses, CPM 2004). 
	/*!
	...
	*
	*/
	// TODO: implement an iterator for the ones in the nearest neightbour dictionary!!! used in the construction of the balanced parentheses support
	...
	public:
		...
		*/
		nearest_neighbour_dictionary(const bit_vector &v) :m_ones(0), m_size(0) {
			...
			// initialize absolue samples m_abs_samples[0]=0
		...
		*/
		size_type rank(size_type idx)const {
			...
			size_type i = (size_type)m_abs_samples[r];
			while (++result <= m_ones) {
				if ((result % sample_dens) == 0) {
					i = (size_type)m_abs_samples[result / sample_dens];
				}
				else {
					i = i + (size_type)m_differences[result - result / sample_dens - 1];
				}
				if (i >= idx)
					return result - 1;
			}
		...
		*/
		size_type select(size_type i)const {
			...
			size_type result = (size_type)m_abs_samples[j];
			j = j*sample_dens - j;
			for (size_type end = j + (i%sample_dens); j < end; ++j) {
				result += (size_type)m_differences[j];
			}
			return result;
		}
